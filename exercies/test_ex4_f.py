...
from ex4_e import (
    build_bitmap_index_for_months_range_coded,
    build_bitmap_index_for_days_range_coded,
)
from ex4_f import count_sales_between_dates_range_coded
import zlib


def test_range_coded_multi_component_bitmap_index(
    database: Database,
    first_month: int,
    first_day: int,
    last_month: int,
    ...
    bitmap_indexes_months_range_coded = build_bitmap_index_for_months_range_coded(
        database
    )
    bitmap_indexes_days_range_coded = build_bitmap_index_for_days_range_coded(database)

    start_time = perf_counter()

    num_sales = count_sales_between_dates_range_coded(
        bitmap_indexes_months_range_coded,
        bitmap_indexes_days_range_coded,
        first_month,
        first_day,
        last_month,
        last_day,
...
def test_range_coded_multi_component_bitmap_index_multiple():
    ...
    for first_month, first_day, last_month, last_day in [
        ...
        test_range_coded_multi_component_bitmap_index(
            database, first_month, first_day, last_month, last_day, cursor
        )

    print(
        "For comparison, our bitmap index query took 0.8 milliseconds "
        "and the SQL query took 55 milliseconds on our test computer."
    )
...
if __name__ == "__main__":
    ...
    test_range_coded_multi_component_bitmap_index_multiple()
